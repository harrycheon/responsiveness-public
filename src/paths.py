...
# directory for paper related scripts and output 
...
def get_rij_file(data_name, action_set_name, model_type, **kwargs):
    assert isinstance(data_name, str) and len(data_name) > 0
    assert isinstance(action_set_name, str) and len(action_set_name) > 0
    f = results_dir / f"{data_name}_{action_set_name}_{model_type}_rij.df"
    return f
