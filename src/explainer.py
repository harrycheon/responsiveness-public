...
class ModelExplainer(ABC):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.exp = None
        self.U, self.inv, self.cnt = np.unique(
            data.X, 
            return_inverse=True, 
            return_counts=True, 
            axis=0
    ...
    def _gen_exp_helper(self, rows):
        ...
        self.exp = {
            'values': exp_df,
            'probas': self.model.predict_proba(self.U),
        }
...
class SHAP_Explainer(ModelExplainer):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.explainer = shap.Explainer(
            model=model, 
            masker=kwargs.get('masker', self.U)
        )  
    ...
    def _generate_exp(self, **kwargs):
        """
        """ 
        self._gen_exp_helper(self.explainer(self.U).values)
