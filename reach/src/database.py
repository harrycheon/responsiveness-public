...
class ReachableSetDatabase:
    ...
    def generate(self, X: Union[np.ndarray, pd.DataFrame], overwrite: bool=False):
        ...
        _, types, types_to_x = np.unique(U[:, mutable], axis=0, return_index=True, return_inverse=True)
        siblings = {i: np.flatnonzero(i == types_to_x) for i in range(len(types))}

        def _store_reachable_set(db, key, x, reachable_set, final_time):
        ...
        with h5py.File(self.path, "a") as db:
            for unique_mutable_idx, sib_idxs in tqdm(siblings.items()):
                ...
                for s in sib_idxs[1:]:
                    ...
                    if overwrite or key not in db:
                        start_time = time.time()
                        reachable_set.X[:, immutable] = U[s, immutable]
                        final_time = time.time() - start_time
                        new_entries.append((key, U[s], reachable_set, final_time))

                out += [_store_reachable_set(db, *entry) for entry in new_entries]
