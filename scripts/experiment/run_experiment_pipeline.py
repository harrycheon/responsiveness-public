...
def main():
    ...
    parser.add_argument(
        "--stages",
        nargs="+",
        default=["setup", "db", "train", "explain", "audit", "plot"],
    )
    ...
    if "audit" in args.stages:
        for model in args.models:
            pipeline.append(
                    f"python scripts/experiment/run_audit.py --data_name={args.data_name} --action_set_name={args.action_set_name} --model_type={model}",
            )
