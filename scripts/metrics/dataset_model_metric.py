...
def main():
    ...
    fixed = denied[~denied['recourse_exists']]
    ...
    cnt_1D = rij_df.groupby('u_index')[['recourse_count']].sum().reset_index()
    not_fixed_df = cnt_1D[cnt_1D['u_index'].isin(denied[denied['recourse_exists']].index)]
    nD_only = not_fixed_df[not_fixed_df['recourse_count'] == 0]
    ...
    action_cnt_df['total_recourse_count'] = action_cnt_df['flip_action_idx'].apply(len)
    action_cnt_df['nD_recourse_count'] = action_cnt_df['total_recourse_count'] - action_cnt_df['recourse_count']

    perct = np.percentile(
        np.repeat(
            action_cnt_df[['recourse_count', 'nD_recourse_count']].values, 
            den_cnts, 
            axis=0
            ), 
        Q, 
