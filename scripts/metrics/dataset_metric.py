...
ass_feats = list([set(action_set.constraints.get_associated_features(f)) for f in act_feats])
...
def main():
    ...
    df = rij_df.groupby('u_index')[['marginal_action_count']].sum()
    ...
    perct = np.percentile(
        np.repeat(n_actions, data.cnt, axis=0), Q, axis=0
    )
